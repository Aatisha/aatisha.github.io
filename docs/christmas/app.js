...
function init() {
  ...
  // If shared link with valid params, go directly to design screen
  if (hasSharedParams) {
    showDesignScreen();
  } else {
    // Ensure landing screen is shown and design screen is hidden
    showLandingScreen();
  }
...
function showLandingScreen() {
  ...
  if (previewSection) {
    previewSection.classList.remove('drawer-open');
  }
}
...
function initDrawerInteractions() {
  ...
  drawerHandle.addEventListener('touchstart', handleTouchStart, {
    passive: true,
  });
  drawerHandle.addEventListener('touchmove', handleTouchMove, {
    passive: false,
  });
  ...
  controlsSection.addEventListener('touchstart', handleDrawerTouchStart, {
    passive: true,
  });
  controlsSection.addEventListener('touchmove', handleDrawerTouchMove, {
    passive: false,
  });
  controlsSection.addEventListener('touchend', handleDrawerTouchEnd, {
    passive: true,
  });
}
...
function handleDrawerTouchStart(e) {
  ...
  const isHandleArea =
    target.closest('.drawer-handle') || target.closest('.drawer-header');
  const isContentArea = target.closest('.drawer-content');
  const drawerContent = document.querySelector('.drawer-content');

  // When collapsed, only allow drag from handle/header
  if (!isDrawerExpanded && !isHandleArea) return;

  // When expanded, check if we should allow close gesture
  if (isDrawerExpanded) {
    if (isHandleArea) {
      // Always allow close from handle/header
      canCloseBySwipe = true;
    } else if (isContentArea && drawerContent) {
      // Only allow close from content if scrolled to top
      canCloseBySwipe = drawerContent.scrollTop <= 0;
    } else {
...
function handleDrawerTouchMove(e) {
  ...
  // Only handle close gesture if allowed and swiping down
  if (isDrawerExpanded && deltaY > 0 && canCloseBySwipe) {
  ...
  // If not allowed to close by swipe, let the content scroll naturally
}
...
function handleDrawerTouchEnd(e) {
  ...
  if (isDrawerDragging && canCloseBySwipe) {
...
function updateThumbnailActiveStates() {
  ...
  document.querySelectorAll('#view-selector .thumbnail-btn').forEach((btn) => {
    btn.classList.toggle(
      'active',
      btn.dataset.value === currentSelections.view
    );
  });
  ...
  document.querySelectorAll('#chair-selector .thumbnail-btn').forEach((btn) => {
    btn.classList.toggle(
      'active',
      btn.dataset.value === currentSelections.chair
    );
  });
  ...
  document.querySelectorAll('#table-selector .thumbnail-btn').forEach((btn) => {
    btn.classList.toggle(
      'active',
      btn.dataset.value === currentSelections.table
    );
  });
}
